...
const AccountCard = ({ type, amount }) => {
  return (
    <div>
      <h1 className="text-[18px] font-semibold text-white mt-4 ml-2">{type}</h1>
      <p className="text-[24px] font-bold text-white mt-2 ml-2">{amount}</p>
    </div>
  );
};
...
const Dashboard = () => {
  ...
  return (
    <div
      className={`w-full mt-6 transition-opacity duration-500 z-0 ${
        isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
      }`}
    >
      <div className="flex justify-between text-[#49505A]">
        {user ? (
          <p className="md:text-[20px] text-[16px]">Welcome, {user.email}</p>
        ) : (
          <p>Please log in</p>
        )}
        <p className="text-[#49505A] md:text-[20px] text-[14px] hidden md:block">
          ...
          <div className="flex gap-6 flex-col xl:flex-row mt-2 w-full">
            ...
            {dashboardData.map((data, index) => (
              <div
                ...
                data-aos-duration="1000"
                key={index}
                className="bg-[#034AB3] rounded-[8px] px-4 py-4 h-[142px] w-full mx-auto mb-4 mt-4 items-center xl:w-[400px] 2xl:w-[450px]"
              >
                <AccountCard type={data.type} amount={data.amount} />
              </div>
            ))}
          </div>
          <div className="flex justify-between items-center px-3">
            <p className="text-[#121417] text-[20px]">Transaction</p>
            <button className="bg-[#034AB3] gap-1 text-white flex items-center rounded-[4px] px-2 py-1 transition-transform transform hover:scale-105">
              Refresh <BiRefresh className="hover:animate-spin" />
            </button>
          </div>
